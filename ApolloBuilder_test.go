...
func TestRedeemerCollect(t *testing.T) {
	...
	if built.GetTx().TransactionBody.Fee != 257866 {
...
func TestMapPDDecodeEncode(t *testing.T) {
	...
	// Check that round-trip preserves the data (semantic equivalence)
...
func TestMapPdRecode(t *testing.T) {
	for i := range 100 {
		...
		// Check semantic equivalence (since CBOR canonical form may differ)
		if pd.TagNr != newPd.TagNr || pd.PlutusDataType != newPd.PlutusDataType {
...
func TestScriptDataHashConway(t *testing.T) {
	...
	if hex.EncodeToString(
		hash.Payload,
	) != "bb6c3e1cdb24b5c9d0cefe4ffe4804a8822c619810c9ead53315193617809f4f" {
