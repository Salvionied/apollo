...
func (tx *Transaction) UnmarshalCBOR(data []byte) error {
	...
	// Support the StructAsArray top-level form by unwrapping a simple
	// array into the expected map-based decode paths below. This keeps
	// backwards compatibility with existing tagged/untagged encodings.
	if arr, ok := decoded.([]interface{}); ok && len(arr) >= 3 {
		// reuse existing array handling below
		// no-op here; the array branch follows further down
	} else if arr, ok := decoded.([]any); ok && len(arr) >= 3 {
		decoded = arr
	}
