...
func TestPoolRegistrationWithRelaysRoundTrip(t *testing.T) {
	...
	params := PoolParams{
		...
		Relays: RelayPkg.Relays{
			RelayPkg.SingleHostAddr{
				Port: uint16Ptr(3000),
				Ipv4: []byte{1, 2, 3, 4},
				Ipv6: nil,
			},
			RelayPkg.SingleHostName{
				Port:    uint16Ptr(6000),
...
func TestStakeVoteRegDelegCombinations(t *testing.T) {
	...
	_ = roundTrip(
		t,
		StakeVoteDelegCert{Stake: stake, PoolKeyHash: pool, Drep: drep},
	)
	_ = roundTrip(
		t,
		StakeRegDelegCert{Stake: stake, PoolKeyHash: pool, Coin: 1},
	)
...
func TestUnmarshalCert_PoolRegistration_InvalidRelays(t *testing.T) {
	...
	params := []any{
		...
		[]any{
			...
			[]any{
				invalidRelay,
			}, // Relays array with one invalid relay
...
func FuzzUnmarshalCert(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, data []byte) {
		...
		if cert.Kind() != cert2.Kind() {
			t.Fatalf(
				"kind mismatch after round-trip: %d vs %d",
				cert.Kind(),
				cert2.Kind(),
			)
		}
...
func FuzzCertificatesUnmarshal(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, data []byte) {
		...
		if len(cs) != len(cs2) {
			t.Fatalf(
				"length mismatch after round-trip: %d vs %d",
				len(cs),
				len(cs2),
			)
		}
