...
func TestUnmarshalCert_WrongLengths(t *testing.T) {
	...
	wrong, _ = cbor.Encode([]any{uint64(10), StakeCredential{}, serialization.PubKeyHash{}})
	if _, err := UnmarshalCert(wrong); err == nil {
		t.Fatalf("expected error for stake_vote_deleg_cert wrong length")
	}
	// StakeRegDelegCert requires 4
	wrong, _ = cbor.Encode([]any{uint64(11), StakeCredential{}, serialization.PubKeyHash{}})
	if _, err := UnmarshalCert(wrong); err == nil {
	...
	wrong, _ = cbor.Encode([]any{uint64(13), StakeCredential{}, serialization.PubKeyHash{}, Drep{}})
	if _, err := UnmarshalCert(wrong); err == nil {
...
func TestUnmarshalCert_WrongTypes(t *testing.T) {
	...
	bz2, _ := cbor.Encode([]any{uint64(16), StakeCredential{}, int64(1), "bad-anchor"})
	...
	bz3, _ := cbor.Encode([]any{uint64(10), StakeCredential{}, "bad-pool", Drep{}})
