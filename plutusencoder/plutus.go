...
func CborUnmarshal(data string, v any, network byte) error {
	...
	if expectAddress {
		...
		hasBytes = func(p PlutusData.PlutusData) bool {
			...
			if p.PlutusDataType == PlutusData.PlutusMap || p.PlutusDataType == PlutusData.PlutusIntMap {
		...
		dataPd := root
		if dataPd.PlutusDataType != PlutusData.PlutusArray || dataPd.TagNr != 121 {
			if nested, ok := find(dataPd, 0); ok {
				// debug: nested address found (suppressed)
				dataPd = nested
			} else {
				// debug: no nested address found (suppressed)
				// continue and let UnmarshalPlutus attempt to decode addresses
			}
		}
		switch dataPd.Value.(type) {
		case PlutusData.PlutusDefArray:
		case PlutusData.PlutusIndefArray:
		default:
			// debug: address found but value type unexpected (suppressed)
			// continue and let UnmarshalPlutus handle unexpected shapes
		}
	}
...
func anyToPlutusData(v any) PlutusData.PlutusData {
	switch val := v.(type) {
	case cbor.Tag:
		switch val.Number {
		case 121:
			// PlutusBool false or PlutusList/tagged array
			if val.Content == nil {
				return PlutusData.PlutusData{PlutusDataType: PlutusData.PlutusInt, Value: int64(0), TagNr: 121}
			}
			contentPd := anyToPlutusData(val.Content)
			pd := PlutusData.PlutusData{PlutusDataType: PlutusData.PlutusArray, Value: contentPd.Value, TagNr: 121}
		...
		case 122:
			...
			if val.Content == nil {
				return PlutusData.PlutusData{PlutusDataType: PlutusData.PlutusInt, Value: int64(1), TagNr: 122}
			} else {
				contentPd := anyToPlutusData(val.Content)
				pd := PlutusData.PlutusData{PlutusDataType: PlutusData.PlutusArray, Value: contentPd.Value, TagNr: 122}
				if b, err := cbor.Encode(val); err == nil {
					pd.Raw = b
				}
				return pd
		...
		case 123:
			// PlutusList def — but some historical encodings put a map under this tag.
			// If the tagged content decodes to a map, treat it as a PlutusMap
			// (preserve TagNr) so Unmarshal paths expecting maps succeed.
			contentPd := anyToPlutusData(val.Content)
			// If the decoded content is an array, convert to DefArray as before
			if arr, ok := contentPd.Value.(PlutusData.PlutusIndefArray); ok {
				pd := PlutusData.PlutusData{PlutusDataType: PlutusData.PlutusArray, Value: PlutusData.PlutusDefArray(arr), TagNr: 123}
				if b, err := cbor.Encode(val); err == nil {
					pd.Raw = b
				}
				return pd
			...
			case map[string]PlutusData.PlutusData:
				pd := PlutusData.PlutusData{PlutusDataType: PlutusData.PlutusMap, Value: m, TagNr: 123}
				if b, err := cbor.Encode(val); err == nil {
					pd.Raw = b
				}
				return pd
			case map[serialization.CustomBytes]PlutusData.PlutusData:
				return PlutusData.PlutusData{PlutusDataType: PlutusData.PlutusMap, Value: m, TagNr: 123}
			}
			// Fallback: return as array with the decoded value (previous behaviour)
			pd := PlutusData.PlutusData{PlutusDataType: PlutusData.PlutusArray, Value: contentPd.Value, TagNr: 123}
			if b, err := cbor.Encode(val); err == nil {
				pd.Raw = b
			}
			return pd
		case 124:
			...
			if bigInt, ok := contentPd.Value.(big.Int); ok {
				pd := PlutusData.PlutusData{PlutusDataType: PlutusData.PlutusBigInt, Value: bigInt, TagNr: 124}
				if b, err := cbor.Encode(val); err == nil {
					pd.Raw = b
				}
				return pd
			}
			pd := PlutusData.PlutusData{PlutusDataType: PlutusData.PlutusBigInt, Value: big.Int{}, TagNr: 124}
			if b, err := cbor.Encode(val); err == nil {
				pd.Raw = b
			}
			return pd
		case 125:
			...
			if bigInt, ok := contentPd.Value.(big.Int); ok {
				bigInt.Neg(&bigInt)
				pd := PlutusData.PlutusData{PlutusDataType: PlutusData.PlutusBigInt, Value: bigInt, TagNr: 125}
				if b, err := cbor.Encode(val); err == nil {
					pd.Raw = b
				}
				return pd
			}
			pd := PlutusData.PlutusData{PlutusDataType: PlutusData.PlutusBigInt, Value: big.Int{}, TagNr: 125}
			if b, err := cbor.Encode(val); err == nil {
				pd.Raw = b
			}
			return pd
		case 126:
			...
			if bytes, ok := contentPd.Value.([]byte); ok {
				pd := PlutusData.PlutusData{PlutusDataType: PlutusData.PlutusBytes, Value: bytes, TagNr: 126}
				if b, err := cbor.Encode(val); err == nil {
					pd.Raw = b
				}
				return pd
			}
			pd := PlutusData.PlutusData{PlutusDataType: PlutusData.PlutusBytes, Value: []byte{}, TagNr: 126}
			if b, err := cbor.Encode(val); err == nil {
				pd.Raw = b
			}
			return pd
		case 127:
			...
			contentPd := anyToPlutusData(val.Content)
			if bytes, ok := contentPd.Value.([]byte); ok {
				pd := PlutusData.PlutusData{PlutusDataType: PlutusData.PlutusBytes, Value: bytes, TagNr: 127}
				if b, err := cbor.Encode(val); err == nil {
					pd.Raw = b
				}
				return pd
			}
			pd := PlutusData.PlutusData{PlutusDataType: PlutusData.PlutusBytes, Value: []byte{}, TagNr: 127}
			if b, err := cbor.Encode(val); err == nil {
				pd.Raw = b
			}
			return pd
		default:
			// For unknown tags, preserve the tag number and keep the underlying content type
			// so that tagged integers remain integers, tagged bytes remain bytes, etc.
			contentPd := anyToPlutusData(val.Content)
			contentPd.TagNr = val.Number
			return contentPd
		}
	case []interface{}:
		arr := make(PlutusData.PlutusIndefArray, len(val))
		for i, item := range val {
			arr[i] = anyToPlutusData(item)
		}
		pd := PlutusData.PlutusData{PlutusDataType: PlutusData.PlutusArray, Value: arr}
		if b, err := cbor.Encode(val); err == nil {
			pd.Raw = b
		}
		return pd
	case map[interface{}]interface{}:
		m := make(map[serialization.CustomBytes]PlutusData.PlutusData)
		for k, v := range val {
			var keyBytes []byte
			switch kt := k.(type) {
			case string:
			...
			case []byte:
				keyBytes = kt
			default:
				// Try to encode then decode the key to extract a primitive value
				kb, _ := cbor.Encode(k)
				var kv any
				_, _ = cbor.Decode(kb, &kv)
				switch kv2 := kv.(type) {
				case []byte:
					keyBytes = kv2
				case string:
					keyBytes = []byte(kv2)
				default:
					// Fallback to the encoded form if we couldn't obtain a primitive
					keyBytes = kb
				}
			}
			// Build CustomBytes key using the hex encoding of the raw key bytes
			cb := serialization.CustomBytes{Value: hex.EncodeToString(keyBytes)}
			child := anyToPlutusData(v)
		...
		pd := PlutusData.PlutusData{PlutusDataType: PlutusData.PlutusMap, Value: m}
		if b, err := cbor.Encode(val); err == nil {
			pd.Raw = b
		}
		return pd
	case int64:
		return PlutusData.PlutusData{PlutusDataType: PlutusData.PlutusInt, Value: val}
	case uint64:
		return PlutusData.PlutusData{PlutusDataType: PlutusData.PlutusInt, Value: int64(val)}
	case big.Int:
		return PlutusData.PlutusData{PlutusDataType: PlutusData.PlutusBigInt, Value: val}
	case []byte:
		return PlutusData.PlutusData{PlutusDataType: PlutusData.PlutusBytes, Value: val}
	case string:
		return PlutusData.PlutusData{PlutusDataType: PlutusData.PlutusBytes, Value: []byte(val)}
	case interface{ Bytes() []byte }:
		return PlutusData.PlutusData{PlutusDataType: PlutusData.PlutusBytes, Value: val.Bytes()}
	case nil:
		return PlutusData.PlutusData{PlutusDataType: PlutusData.PlutusArray, Value: PlutusData.PlutusIndefArray{}}
	default:
		if v == nil {
			return PlutusData.PlutusData{PlutusDataType: PlutusData.PlutusArray, Value: PlutusData.PlutusIndefArray{}}
		}
...
func unmarshalPlutus(
	...
	if ok {
				...
				for k, v := range mapData {
					...
					if tps.Field(idx).Tag.Get("plutusType") == "HexString" {
						if pAEl.PlutusDataType != PlutusData.PlutusBytes {
							return errors.New("error: HexString field is not bytes")
						}
						bytesVal, ok := pAEl.Value.([]uint8)
						if !ok {
							return fmt.Errorf("error: HexString field value not []uint8 (got %T)", pAEl.Value)
						}
					...
					if tps.Field(idx).Type.String() == "Address.Address" {
						...
						reflect.ValueOf(v).Elem().Field(idx).Set(reflect.ValueOf(addr))
						continue
					}
					if tps.Field(idx).Type.String() == "plutusencoder.Asset" {
						asset := DecodePlutusAsset(pAEl)
						reflect.ValueOf(v).Elem().Field(idx).Set(reflect.ValueOf(asset))
						continue
					}
					if tps.Field(idx).Type.String() == "bool" {
						...
						reflect.ValueOf(v).Elem().Field(idx).SetBool(pAEl.TagNr == 122)
						continue
					}
					...
					case PlutusData.PlutusBytes:
						bytesVal, ok := pAEl.Value.([]uint8)
						if !ok {
							return fmt.Errorf("error: Bytes field is not a slice (got %T)", pAEl.Value)
						}
						...
						reflect.ValueOf(v).Elem().Field(idx).Set(reflect.ValueOf(bytesVal))
					case PlutusData.PlutusInt:
						if tps.Field(idx).Type.String() != "int64" && tps.Field(idx).Type.String() != "uint64" {
							return errors.New("error: Int field is not int64 or uint64")
						}
						x3, ok := pAEl.Value.(int64)
						if !ok {
							y, ok := pAEl.Value.(uint64)
							if !ok {
								return errors.New("error: Int Data field is not int64 or uint64")
							}
							x3 = int64(y)
						}
					...
					case PlutusData.PlutusBigInt:
						if tps.Field(idx).Type.String() != "int64" && tps.Field(idx).Type.String() != "*big.Int" {
							return errors.New("error: Int field is not bigInt")
						}
						x, ok := pAEl.Value.(big.Int)
						if !ok {
							return errors.New("error: Int Data field is not bigInt")
						}
						if tps.Field(idx).Type.String() == "*big.Int" {
							reflect.ValueOf(v).Elem().Field(idx).Set(reflect.ValueOf(&x))
							continue
						}
						i64 := x.Int64()
					...
					case PlutusData.PlutusArray:
						...
						if tps.Field(idx).Type.Kind() == reflect.Slice {
							...
							newSlice := reflect.MakeSlice(tps.Field(idx).Type, 0, 0)
							if arr, ok := pAEl.Value.(PlutusData.PlutusIndefArray); ok {
								for _, el := range arr {
									...
									newSlice = reflect.Append(newSlice, reflect.ValueOf(newEl).Elem())
								}
							}
							reflect.ValueOf(v).Elem().Field(idx).Set(newSlice)
						}
					default:
						return fmt.Errorf("unsupported PlutusDataType for field %s: %v", fieldName, pAEl.PlutusDataType)
					}
				}
				return nil
			}
		...
		case PlutusData.PlutusMap:
			...
			// produced by anyToPlutusData. Iterate keys in a unified way by
			// extracting the hex string for each key.
			...
			errIter := iter(func(idxStringHex string, pAEl PlutusData.PlutusData) error {
					...
					for i := range tps.NumField() {
						...
						if tps.Field(i).Tag.Get("plutusKey") == "" {
							...
							if err := cb.UnmarshalCBOR(idxBytes); err == nil {
								if tps.Field(i).Tag.Get("plutusKey") == cb.HexString() {
									idx = tps.Field(i).Name
									field = tps.Field(i)
									found = true
									break
